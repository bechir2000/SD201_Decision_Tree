...
class Tree:
    ...
    def __init__(self,
        ...
        best_feat = self.points.get_best_gain(min_split_points)[0]
        if best_feat is None or h == 0:
            # No more splitting possible
            self.left = None
            self.right = None
        else:
            self.best_ind, self.split_value, true_class_features, true_class_labels,false_class_features,false_class_lables  = self.points.split_with_best_gain(min_split_points)
        
            self.right=Tree(false_class_features,false_class_lables,types,h-1,min_split_points)
            self.left=Tree(true_class_features,true_class_labels,types,h-1,min_split_points)
            self.node = best_feat
